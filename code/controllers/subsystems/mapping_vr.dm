...
/datum/controller/subsystem/mapping/proc/loadEngine()
	...
	if (config.engine_map && config.engine_map != "Random")
		chosen_type = map_templates[config.engine_map]
		if(!istype(chosen_type))
			error("Configured engine map [config.engine_map] is not a valid engine map name!")
	if(!istype(chosen_type))
